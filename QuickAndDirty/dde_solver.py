...
class DDESolver():
    ...
    def integrate(self, func, ts, history_func):
        ...
        # interpolator class
        y0 = torch.unsqueeze(history_func(ts[0]).clone().detach(), 1)
        ...
        def ode_func(t, y):
            ...
            history = [ys_interpolation(t - tau) if t-tau >= ts[0] else history_func(t-tau) for tau in self.delays]
