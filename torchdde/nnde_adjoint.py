            ...
            with torch.enable_grad():
                ...
                for tau_i in ctx.func.delays:
                    if t < T - tau_i:
                        ...
                        h_t_plus_tau = history[index_de_h_t] # state_interpolator(t + tau_i)
                        # out_other = ctx.func(t + tau_i, h_t_plus_tau, history=[h_t_plus_tau, h_t_plus_tau]) # doesn't work
                        out_other = ctx.func(t + tau_i, h_t_plus_tau, history=[h_t, h_t]) #  work
                        # out_other = ctx.func(t + tau_i, history[index_de_h_t], history=history) # doesnt works
                        rhs_adjoint_inc_k1 = torch.autograd.grad(
