...
from torchdde.step_size_controller import AdaptiveStepSizeController
...
def test_explicit_adaptive_solver1(solver):
    vf = lambda t, y, args: -y
    ts = torch.linspace(0, 5, 500)
    y0 = torch.rand((10, 1))
    stepsize_controller = AdaptiveStepSizeController(1e-3, 1e-6)
    ys = integrate(vf, solver, ts[0], ts[-1], ts, y0, None, stepsize_controller)
    assert torch.allclose(ys[:, -1], y0 * torch.exp(-ts[-1]), rtol=10e-3, atol=10e-3)


@pytest.mark.skip(
    reason="adaptive step size doesnt work with vf that have only the variable t ..."
)
# @pytest.mark.parametrize("solver", [Dopri5()])
def test_explicit_adaptive_solver2(solver):
    vf = lambda t, y, args: t + t**2
    ts = torch.linspace(0, 5, 500)
    y0 = torch.rand((10, 1))
    stepsize_controller = AdaptiveStepSizeController(1e-3, 1e-6)
